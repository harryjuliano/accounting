...
class ChartOfAccountController extends Controller
{
    ...
    public function importMasterTemplate(Request $request)
    {
        $payload = $request->validate([
            'company_id' => ['required', 'integer', 'exists:companies,id'],
        ]);
        ...
        $companyId = (int) $payload['company_id'];
        $this->enforceCompanyAccess($companyId);
        $this->ensureNoMasterCoaExists($companyId);

        $groupDefinitions = $this->defaultAccountGroupDefinitions();
        $groups = collect($groupDefinitions)->mapWithKeys(function (array $group, string $key) use ($companyId) {
        ...
        $template = $this->defaultMasterTemplate();
        $coaByCode = [];

        DB::transaction(function () use ($template, &$coaByCode, $groups, $companyId): void {
        ...
        return back()->with('success', 'Berhasil import template Master COA.');
    }
    ...
    private function ensureNoMasterCoaExists(int $companyId): void
    {
        $hasMasterCoa = ChartOfAccount::query()
            ->where('company_id', $companyId)
            ->where('level', '<=', 3)
            ->exists();
        ...
        if ($hasMasterCoa) {
            throw ValidationException::withMessages([
                'company_id' => 'Import template Master COA hanya bisa dilakukan ketika Master COA level 1-3 masih kosong.',
            ]);
        }
    }
    ...
    private function defaultAccountGroupDefinitions(): array
    {
        return [
    ...
    private function defaultMasterTemplate(): array
    {
        return [
