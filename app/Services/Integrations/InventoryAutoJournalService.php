...
class InventoryAutoJournalService
{
    ...
    public function postValidatedEvent(IntegrationEvent $event): array
    {
        ...
        $this->lifecycle->log($event, 'info', 'Auto posting started.', [
        ...
        $integrationKey = 'inventory:event:' . $event->id;
    ...
    private function markFailed(IntegrationEvent $event, string $error): array
    {
        ...
        $this->lifecycle->recordFailure($event, 'posting', $error, 'Inventory auto posting failed: ' . $error);
        $this->lifecycle->log($event, 'error', 'Auto posting failed.', [
            'error_code' => $error,
        ]);
