...
class ModulePresetAutoJournalService
{
    ...
    public function postValidatedEvent(IntegrationEvent $event): array
    {
        ...
        $this->lifecycle->log($event, 'info', 'Module preset auto posting started.', [
        ...
        $integrationKey = $event->source_module . ':event:' . $event->id;
        ...
        if ($existing) {
            $this->lifecycle->log($event, 'warning', 'Duplicate module preset auto posting detected. Existing journal reused.', [
        ...
        if ($exchangeRate <= 0) {
            return $this->markFailed($event, 'invalid_exchange_rate');
        }
        ...
        $this->lifecycle->log($event, 'info', 'Module preset auto posting completed.', [
            'journal_entry_id' => $journalEntry->id,
    ...
    private function resolveTotalsAndValidateAccounts(array $lines, int $companyId): array
    {
        ...
        foreach ($lines as $line) {
            ...
            if (($line['line_side'] ?? null) === 'debit') {
                $totalDebit += $amount;
            } elseif (($line['line_side'] ?? null) === 'credit') {
                $totalCredit += $amount;
            } else {
                return [0, 0, 'invalid_line_side'];
            }
        }
    ...
    private function generateJournalNumber(IntegrationEvent $event): string
    {
        ...
        $modulePrefix = Str::of($event->source_module)->upper()->replaceMatches('/[^A-Z0-9]+/', '-')->trim('-')->limit(20, '');
        $prefix = $modulePrefix . '-AUTO-' . $datePart . '-';
    ...
    private function markFailed(IntegrationEvent $event, string $error): array
    {
        ...
        $this->lifecycle->recordFailure($event, 'posting', $error, 'Module preset auto posting failed: ' . $error);
        $this->lifecycle->log($event, 'error', 'Module preset auto posting failed.', [
            'error_code' => $error,
        ]);
