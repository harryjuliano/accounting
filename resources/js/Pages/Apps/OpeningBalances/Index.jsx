...
export default function Index() {
    ...
    const submit = (e) => {
        e.preventDefault();
        post(data.isUpdate ? route('apps.opening-balances.update', data.id) : route('apps.opening-balances.store'), { onSuccess: resetForm });
    };
    ...
    const applyListFilters = React.useCallback((nextFilters) => {
        router.get(route('apps.opening-balances.index'), {
            ...nextFilters,
    ...
    const bulkPostSelected = () => {
        ...
        router.post(route('apps.opening-balances.bulk-post'), {
    ...
    const submitImport = (event) => {
        event.preventDefault();
        postImport(route('apps.opening-balances.import'), {
            forceFormData: true,
    ...
    const downloadImportTemplate = () => {
        window.location.href = route('apps.opening-balances.import-template');
    };
        ...
        <>
            ...
            <div className='mb-2 flex justify-between items-center gap-2'>
                <div className='flex items-center gap-2'>
                    <Button type='button' icon={<IconCirclePlus size={20} strokeWidth={1.5} />} variant='gray' label='Tambah Saldo Awal' onClick={() => setData('isOpen', true)} />
                    <Button type='button' icon={<IconFileImport size={20} strokeWidth={1.5} />} variant='gray' label='Import Saldo Awal' onClick={openImportModal} />
                    <Button type='button' icon={<IconFileSpreadsheet size={20} strokeWidth={1.5} />} variant='gray' label='Download Template Saldo Awal' onClick={downloadImportTemplate} />
                </div>
            ...
            <Modal
                ...
                onClose={resetForm}
                title={data.isUpdate ? 'Ubah Saldo Awal' : 'Tambah Saldo Awal'}
                    ...
                    <Table.Tbody>
                        {manualJournals.data.length ? manualJournals.data.map((journal, i) => (
                            <tr key={journal.id} className='hover:bg-gray-100 dark:hover:bg-gray-900'>
                                ...
                                <Table.Td><div className='flex gap-2'><Button type='modal' variant='orange' icon={<IconPencilCog size={16} strokeWidth={1.5} />} onClick={() => openJournalEditor(journal)} /><Button type='delete' variant='rose' icon={<IconTrash size={16} strokeWidth={1.5} />} url={route('apps.opening-balances.destroy', journal.id)} /></div></Table.Td>
                            </tr>
                        )) : <Table.Empty colSpan={12} message={<><div className='flex justify-center mb-2'><IconDatabaseOff size={24} /></div><span>Data saldo awal tidak ditemukan.</span></>} />}
                    </Table.Tbody>
                </Table>
            </Table.Card>
